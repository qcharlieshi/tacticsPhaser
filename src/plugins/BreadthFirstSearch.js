...
export default class BreadthFirstSearch extends Phaser.Plugin {
    ...
    check_neighbor (position, positions_to_check, visited_positions) {
        //add following code for collision map
        //&& !this.map.getTileWorldXY(position.x, position.y, this.tile_dimensions.x, this.tile_dimensions.y, "collision"
    ...
    add_neighbors (position, positions_to_check, visited_positions) {
        ...
        this.check_neighbor(left_neighbor, positions_to_check, visited_positions);

        right_neighbor = new Phaser.Point(position.x + this.tile_dimensions.x, position.y);
        this.check_neighbor(right_neighbor, positions_to_check, visited_positions);

        top_neighbor = new Phaser.Point(position.x, position.y - this.tile_dimensions.y);
        this.check_neighbor(top_neighbor, positions_to_check, visited_positions);

        bottom_neighbor = new Phaser.Point(position.x, position.y + this.tile_dimensions.y);
        this.check_neighbor(bottom_neighbor, positions_to_check, visited_positions);
    }
